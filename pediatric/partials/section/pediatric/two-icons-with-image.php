    ...
    <div class="content">
        <div class="inner-content">
            <div class="image-container">
                <img src="<?= !empty($image['src']) ? $image['src'] : '';?>"<?= !empty($image['alt']) ? 'alt="' . $image['alt'] . '"' : '';?><?= !empty($image['classes']) ? 'class="' . implode(' ', $image['classes']) . '"' : '';?>>
            </div>
            <div class="content-box">
                <? if(!empty($heading)) : ?>
                    <h3<?= !empty($heading_classes) ? ' class="' . implode(' ', $heading_classes) . '"' : '';?>><?= $heading; ?></h3>
                <? endif; ?>
                <? if($copy) : ?>
                    <?= $copy; ?>
                <? endif; ?>
